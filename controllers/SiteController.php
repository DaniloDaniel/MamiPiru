...
class SiteController extends Controller
{
    ...
    public function actionRegister()
    {
        ...
        if ($model->load(Yii::$app->request->post()))
        {
            if($model->validate())
            {
                ...
                if ($table->insert())
                {           
                    $msg = "Felicitaciones, su cuenta se ha creado exitosamente";
                }
                else
                {
    ...
    // VERIFY USER AND SEND RECOVERY LINK TO EMAIL
    // REMEMBER TO UPDATE THE REDIRECT LINK ON THE EMAIL EVERY TIME THE SERVER CHANGES
        ...
        if ($model->load(Yii::$app->request->post()))
        {
            if ($model->validate())
            {
                ...
                if ($table->count() == 1)
                {
                    ...
                    $body .= "<p><a href='http://localhost/mamipiru/web/index.php?r=site%2Fresetpass'>Recuperar password</a></p>";
    ...
    // RESET PASSWORD FORM, VALIDATE FORM AND UPDATE DB, THEN REDIRECT TO LOGIN
